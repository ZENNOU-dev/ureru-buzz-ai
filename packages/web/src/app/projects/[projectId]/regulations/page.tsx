...
import { ArrowLeft, CheckCircle2, XCircle, AlertTriangle, Plus, X, Tag, Search, FileText, Video } from "lucide-react";
import Link from "next/link";

// ─── Types ────────────────────────────────────────────
type CheckStatus = "確認依頼" | "承認済" | "要修正";
type CheckResult = "ok" | "ng" | "pending";
...
type KnowledgeItem = {
  ...
  ngExpression: string;
  correction: string;
  source: string; // "薬機法" | "景表法" | "クライアント指摘" etc.
  createdAt: string;
};
...
// ─── Sample Data ──────────────────────────────────────
function makeScriptChecks(): SceneCheck[] {
  return [
    { id: 1, sceneNum: 1, section: "フック1", text: "未経験なら\nAIフリーランス\nめちゃチャンスです🥺", result: "ng", issues: [
      { id: 1, category: "景表法", severity: "warning", description: "「めちゃチャンス」は誇大表現の可能性", suggestion: "「チャンスが広がっています」に変更" },
    ]},
    { id: 2, sceneNum: 2, section: "フック2", text: "AIのおかげで\n収入変わりすぎて\n正直びっくり🤩", result: "ng", issues: [
    ...
    { id: 3, sceneNum: 3, section: "フック3", text: "給料・職場環境\n変わらない\n会社員辞めて", result: "ok", issues: [] },
    { id: 4, sceneNum: 4, section: "フック3", text: "AIフリーランスに\n転身したら・・・", result: "ok", issues: [] },
    { id: 5, sceneNum: 5, section: "フック3", text: "バブルすぎて\nびっくりしてます😭", result: "ng", issues: [
      { id: 4, category: "景表法", severity: "error", description: "「バブルすぎて」は収益を誇大に表現", suggestion: "「想像以上の成果が出て」に変更" },
    ]},
    { id: 6, sceneNum: 6, section: "商品紹介", text: "私は\nAIとXを使って\n会社員時代の月収を上回る", result: "ng", issues: [
      { id: 5, category: "景表法", severity: "error", description: "「月収を上回る」は具体的な収益を示唆", suggestion: "「新たな収入源を確保」に変更" },
    ]},
    { id: 7, sceneNum: 7, section: "商品紹介", text: "初めは\nAI使わなきゃな〜\nって", result: "ok", issues: [] },
    { id: 8, sceneNum: 8, section: "商品紹介", text: "チャッピー愛用\nしてたけど", result: "ok", issues: [] },
    { id: 9, sceneNum: 9, section: "権威性", text: "マーケで成功してる\n連続起業家\n発案だから", result: "warning" as CheckResult, issues: [
      { id: 6, category: "クライアント固有", severity: "warning", description: "「成功してる」の根拠が不明確", suggestion: "具体的な実績を記載するか表現を緩和" },
    ]},
    { id: 10, sceneNum: 10, section: "CTA", text: "3ヶ月で\n会社員時代の\n収入額更新✨", result: "ng", issues: [
      { id: 7, category: "景表法", severity: "error", description: "「3ヶ月で収入額更新」は期間と効果を断定", suggestion: "「※個人の体験であり効果を保証するものではありません」を追加" },
    ]},
  ];
}

function makeVideoChecks(): SceneCheck[] {
  return [
    { id: 101, sceneNum: 1, section: "フック1", text: "テロップ: 未経験ならAIフリーランス", result: "ok", issues: [] },
    { id: 102, sceneNum: 2, section: "フック2", text: "テロップ: 収入変わりすぎて正直びっくり", result: "ng", issues: [
    ...
    { id: 104, sceneNum: 6, section: "商品紹介", text: "素材: メイン動画", result: "ok", issues: [] },
    { id: 105, sceneNum: 10, section: "CTA", text: "注釈サイズ・表示時間", result: "ng", issues: [
...
export default function RegulationsPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [scriptStatus, setScriptStatus] = useState<CheckStatus>("確認依頼");
  const [videoStatus, setVideoStatus] = useState<CheckStatus>("確認依頼");
  const [scriptChecks] = useState(makeScriptChecks);
  const [videoChecks] = useState(makeVideoChecks);
  const [knowledge, setKnowledge] = useState(makeKnowledge);
  const [knowledgeSearch, setKnowledgeSearch] = useState("");
  const [knowledgeTagFilter, setKnowledgeTagFilter] = useState("");

  const scriptOk = scriptChecks.filter((c) => c.result === "ok").length;
  const scriptNg = scriptChecks.filter((c) => c.result === "ng").length;
  const videoOk = videoChecks.filter((c) => c.result === "ok").length;
  const videoNg = videoChecks.filter((c) => c.result === "ng").length;
  ...
  const checks = activeTab === "script" ? scriptChecks : videoChecks;
  const status = activeTab === "script" ? scriptStatus : videoStatus;
  const setStatus = activeTab === "script" ? setScriptStatus : setVideoStatus;
  const okCount = activeTab === "script" ? scriptOk : videoOk;
  const ngCount = activeTab === "script" ? scriptNg : videoNg;
      ...
      <div className="bg-white border-b border-black/[0.06] shrink-0">
        <div className="px-6 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-lg font-bold text-[#1A1A2E]">フリーランス2.0体験談</h1>
          </div>
          {activeTab !== "knowledge" && (
            <select value={status} onChange={(e) => setStatus(e.target.value as CheckStatus)}
              className={`text-[11px] font-semibold px-3 py-1.5 rounded-full border-0 outline-none cursor-pointer appearance-none ${statusOpt.bg} ${statusOpt.text}`}>
        ...
        <div className="px-6 flex gap-0 border-t border-black/[0.03]">
          {[
            { key: "script" as const, label: "台本チェック", icon: FileText, count: scriptChecks.length },
            { key: "video" as const, label: "動画チェック", icon: Video, count: videoChecks.length },
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-1.5 px-4 py-2.5 text-[12px] font-semibold border-b-2 transition-all ${
                activeTab === tab.key
                  ...
                  : "border-transparent text-[#1A1A2E]/30 hover:text-[#1A1A2E]/50"
              }`}>
              <tab.icon className="w-3.5 h-3.5" />
              {tab.label}
              <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-[#9333EA]/10 text-[#9333EA]" : "bg-black/[0.04] text-[#1A1A2E]/30"}`}>
                {tab.count}
              </span>
            </button>
          ))}
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="flex items-center gap-4 bg-white rounded-xl border border-black/[0.06] px-5 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] font-semibold text-amber-600">{checks.filter((c) => c.issues.some((i) => i.severity === "warning")).length} 警告</span>
              </div>
              ...
              {checks.map((check) => {
                ...
                return (
                  <div key={check.id} className={`bg-white rounded-xl border overflow-hidden ${
                    check.result === "ng" ? "border-red-200" : check.result === "ok" ? "border-emerald-100" : "border-black/[0.06]"
                  }`}>
                    {/* Scene header */}
                    <div className={`flex items-center gap-3 px-4 py-2.5 ${
                      check.result === "ng" ? "bg-red-50/50" : check.result === "ok" ? "bg-emerald-50/30" : "bg-white"
                    }`}>
                        ...
                        {check.issues.map((issue) => (
                          <div key={issue.id} className="flex gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[11px] text-[#1A1A2E]/70 leading-relaxed">{issue.description}</p>
